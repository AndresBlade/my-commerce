...
let pageProduct = params.get('pageProduct') != null ? params.get('pageProduct') : 0;
let pageTienda = params.get('pageTienda') != null ? params.get('pageTienda') : 0;
...
function fetchProducto(pageProduct = 0) {
	return fetch(`http://127.0.0.1:3000/api/productos/getProducts?page=${pageProduct}`, {
	  method: 'GET',
	  url: 'http://127.0.0.1:3000',
	})
	  .then(respuesta => {
  ...
  function fetchTienda(pageTienda = 0) {
	return fetch(`http://127.0.0.1:3000/api/tienda/tiendas?page=${pageTienda}`, {
	  method: 'GET',
	  url: 'http://127.0.0.1:3000',
	})
	  .then(respuesta => {
  ...
  function renderizarProductos(productos) {
	...
	productos.forEach(producto => {
	  ...
	  productoHTML.classList.add('producto');
	  const imagenes = producto.imagenes.split(' ');
	  const imagenHTML = `<img class="producto__imagen" src="${imagenes[Math.floor(Math.random() * imagenes.length)]}" />`;
	  productoHTML.innerHTML += imagenHTML;
	  productoHTML.innerHTML += `
		<div class="informacion">
	  ...
	  const url = new URL('http://127.0.0.1/e-commerce-tarea/frontend/vistas/producto.html');
	  url.searchParams.set('id', producto.id);
  ...
  function renderizarTiendas(tiendas) {
	...
	tiendas.forEach(tienda => {
	  ...
	  tiendaHTML.innerHTML += `
		...
		const url = new URL('http://127.0.0.1/e-commerce-tarea/frontend/vistas/tienda.html');
  ...
  const btnPaginaProductosAnterior = document.getElementById('btn-pagina-productos-anterior');
  const btnPaginaProductosSiguiente = document.getElementById('btn-pagina-productos-siguiente');
  ...
  const btnPaginaTiendasAnterior = document.getElementById('btn-pagina-tiendas-anterior');
  const btnPaginaTiendasSiguiente = document.getElementById('btn-pagina-tiendas-siguiente');

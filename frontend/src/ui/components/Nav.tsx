...
export const Nav = () => {
	...
	const bg_dark = showMenu ? 'bg-modal' : 'bg-modal hidden';
	...
	return (
		<div className="flex gap-8 items-center">
			<Link
				className="hidden lg:flex text-blanco text-[1rem] font-medium link-hover-effect"
				to="explorar"
			>
				Explorar
			</Link>
			...
			<Link
				className={`${
					token
						? 'hidden lg:flex cursor-pointer w-14 h-14 bg-gris rounded-full'
						: 'hidden'
				}`}
				to={nombre}
			>
			...
			<ul className={menu}>
				<span
					className="flex justify-center items-center ml-auto mt-2 mr-3 w-[2.5rem] h-[2.5rem] text-[2rem] font-normal text-gray-400 cursor-pointer rounded-lg hover:bg-gray-200"
					onClick={MenuClick}
				>
					×
				</span>
				<Link
					to={nombre}
					onClick={MenuClick}
					className="flex mx-auto w-[95%] rounded-2xl hover:bg-gris"
				>

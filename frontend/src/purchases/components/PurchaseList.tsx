...
export const PurchaseList = () => {
	...
	const generatePDF = () => {
		...
		if (purchases) {
			const columns = ['Nombre', 'Cantidad', 'Precio individual', 'Total'];
			const rows = purchases.map(purchase => [
				purchase.detallesProducto.nombre,
				purchase.detallesVenta.cantidad,
				purchase.detallesProducto.precio,
				(purchase.detallesVenta.cantidad * purchase.detallesProducto.precio),
			]);

			doc.text('MyCommerce', 15, 15); 
				const currentDate = new Date();
				const formattedDate = `${currentDate.getDate()}/${currentDate.getMonth() + 1}/${currentDate.getFullYear()}`;
			doc.text(`Fecha: ${formattedDate}`, doc.internal.pageSize.width - 60, 15);
	...
	return (
		<div className='w-full'>
			<div className="p-5 flex flex-col mx-auto w-full overflow-hidden">

...
export const AuthPage = ({ register }: Props) => {
	...
	return (
		...
		>
			<div className='w-80 py-8 px-4 my-12 mx-auto bg-blanco rounded-[10px] text-center shadow-2xl' style={{ margin: `${0} auto` }}>
				<h1 className='mb-4'>{register ? 'Regístrate' : 'Inicia Sesión'}</h1>
				...
				<div className="text-[.8rem] mt-6">
					{register ? (
						<>
							¿Ya está registrado?
							<Link to="/login" className='text-dark-blue'> Inicia sesión aquí</Link>
						</>
					) : (
						<>
							¿Aún no te has registrado?
							<Link to="/register" className='text-dark-blue'> Regístrate aquí</Link>
						</>
					)}
				</div>
			</div>
